...
const countryCases = async () => {
  ...
  if (response.ok) {
    const data = await response.json();
    const parseData = {
      country: data.Countries.map((country) => country.CountryCode),
      confirmed: data.Countries.map((country) => country.NewConfirmed),
    };
    console.log(parseData);

    $.getJSON("../countryLocation/country-location.json", function (result) {
      console.log(result);
      // for (let i of parseData.country){
      //     for (let j of result.country){
      //         if(i === j){
      //             console.log(j)
      //         }
      //     }
      ...
      // }
      function x(country) {}
      // result.map((item) => console.log(item.country))
      parseData.country.map((item) => console.log(item));
      // if(parseData.country[row] === row.country){
  ...
  var countryLoc = $.getJSON(
    "../countryLocation/country-location.json",
    function (result) {
      countryCoord = result;
